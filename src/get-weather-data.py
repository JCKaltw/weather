...
def get_dirty_locations(db_conn):
    """
    Retrieve all addresses from the weather.weather_location table where dirty = true.
    Returns a list of address strings.
    """
    with db_conn.cursor() as cur:
...
def mark_address_clean(db_conn, address):
    """
    Mark an address as clean (dirty = false) in the weather.weather_location table.
    
    Args:
        db_conn: Database connection
        address: Weather location address to mark as clean
    """
    with db_conn.cursor() as cur:
        cur.execute("UPDATE weather.weather_location SET dirty = false WHERE address = %s", (address,))
        db_conn.commit()
        print(f"Marked address '{address}' as clean (dirty = false)")
...
def get_display_group_time_range(db_conn, display_group_id):
    """
    Get the time range for a specific display group.
    Returns a tuple of (weather_address, start_datetime, end_datetime, timezone_name) or None if not found.
    """
    with db_conn.cursor() as cur:
        # Get display group parameters
        cur.execute("""
        ...
        row = cur.fetchone()
        if row is None:
            return None  # Display group not found or is_removed
        
        test_start_date, test_end_date, day_start_seconds, tz_name, weather_address = row
        
        if not weather_address or not test_start_date:
            return None  # No weather address or test start date configured
        ...
        return weather_address, start_datetime_local, end_datetime_local, tz_name


def merge_overlapping_time_ranges(time_ranges):
    """
    Merge overlapping time ranges into a minimal set of non-overlapping ranges.
    
    Args:
        time_ranges: List of (start_datetime, end_datetime) tuples
    
    Returns:
        List of (start_datetime, end_datetime) tuples with no overlaps
    """
    if not time_ranges:
        return []
...
def check_missing_hours_for_time_ranges(db_conn, weather_address, time_ranges):
    """
    Check for missing hourly weather data within the specified time ranges for a given address.
    Returns a list of (weather_address, missing_hour_timestamp) tuples.
    
    Args:
        db_conn: Database connection
        weather_address: Weather address to check
        time_ranges: List of (start_datetime, end_datetime) tuples to check
    """
    missing_hours = []
    
    with db_conn.cursor() as cur:
        for start_datetime_local, end_datetime_local in time_ranges:
            # Check for missing daily data first
            current_date = start_datetime_local.date()
            end_date = end_datetime_local.date()
            ...
            # For missing daily data, add all hours in the expected range for those dates
            for missing_date in missing_dates:
                # Get timezone from the datetime object
                local_tz = start_datetime_local.tzinfo
                date_start = datetime.combine(missing_date, datetime.min.time())
                date_start_local = local_tz.localize(date_start)
...
def collect_missing_hours_by_address_optimized(db_conn, address, include_inactive=False):
    """
    Collect missing hours for a specific address using optimized time range merging.
    
    This function:
    1. Collects time ranges from all display groups for the address
    2. Merges overlapping time ranges to minimize redundant checks
    3. Checks for missing hours only within the merged ranges
    
    Args:
        db_conn: Database connection
        address: Weather address to check
        include_inactive: If False (default), only consider active display groups.
                         If True, consider all display groups regardless of active status.
    
    Returns:
        Tuple of (display_group_ranges, merged_ranges, missing_hours) where:
        - display_group_ranges: Dict mapping display_group_id to its time range
        - merged_ranges: List of merged (start, end) time ranges
        - missing_hours: List of missing hour timestamps
    """
    # Get all display group IDs for this address
    with db_conn.cursor() as cur:
        if include_inactive:
            cur.execute("""
                SELECT display_group_id 
                FROM display_group 
                WHERE weather_address = %s
                AND (is_removed = false OR is_removed IS NULL)
                ORDER BY display_group_id
            """, (address,))
        else:
    ...
    print(f"Found {len(display_group_ids)} {'display groups' if include_inactive else 'active display groups'} for this address")
    
    # Collect time ranges from all display groups
    display_group_ranges = {}
    all_time_ranges = []
    
    for display_group_id in display_group_ids:
        result = get_display_group_time_range(db_conn, display_group_id)
        if result:
            weather_address, start_datetime, end_datetime, tz_name = result
            display_group_ranges[display_group_id] = (start_datetime, end_datetime)
            all_time_ranges.append((start_datetime, end_datetime))
            print(f"  Display group {display_group_id}: {start_datetime.strftime('%Y-%m-%d %H:%M:%S %Z')} to {end_datetime.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    
    if not all_time_ranges:
        print("  No valid time ranges found for this address")
        return {}, [], []
    
    # Merge overlapping time ranges
    merged_ranges = merge_overlapping_time_ranges(all_time_ranges)
    
    print(f"\nOptimized time ranges (merged from {len(all_time_ranges)} to {len(merged_ranges)} ranges):")
    for start, end in merged_ranges:
        print(f"  {start.strftime('%Y-%m-%d %H:%M:%S %Z')} to {end.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    
    # Check for missing hours only within the merged ranges
    print(f"\nChecking for missing hours within {len(merged_ranges)} merged time ranges...")
    missing_hours_tuples = check_missing_hours_for_time_ranges(db_conn, address, merged_ranges)
    
    # Extract just the timestamps from the tuples
    missing_hours = [timestamp for _, timestamp in missing_hours_tuples]
    
    return display_group_ranges, merged_ranges, missing_hours
...
def fetch_missing_hours_for_single_address(db_conn, api_key, address, dry_run=False, max_days_per_request=None, include_inactive=False):
    """
    ...
    Args:
        db_conn: Database connection
        api_key: Visual Crossing API key
        address: Weather address to fetch missing hours for
        dry_run: If True, show what would be done without executing
        max_days_per_request: Maximum days to fetch in a single request (None = no limit)
        include_inactive: If False (default), only consider active display groups.
                         If True, consider all display groups regardless of active status.
...
def process_dirty_locations(api_key, dry_run=False, max_days_per_request=None, include_inactive=False):
    """
    Process weather data for dirty locations (those marked with dirty = true).
    Fetches missing hourly weather data for each dirty location and marks them clean.
    ...
    try:
        # Get dirty locations from database
        dirty_locations = get_dirty_locations(db_conn)
        print(f"Found {len(dirty_locations)} dirty locations to process")
        
        if not dirty_locations:
            print("No dirty locations found. All locations are clean.")
            return
        
        # Process each dirty location
        for i, address in enumerate(dirty_locations, 1):
            print(f"\n[{i}/{len(dirty_locations)}] Processing dirty location: {address}")
            
            # Fetch missing hours for this address using the shared function
            success = fetch_missing_hours_for_single_address(
                db_conn, api_key, address, dry_run=dry_run, 
                max_days_per_request=max_days_per_request, 
                include_inactive=include_inactive)
...
#------- END NEW --------
    ...
    parser.add_argument("--fetch-missing-hours-for-dirty-addresses", action='store_true',
                        help="Fetch missing hourly weather data for all addresses marked as dirty (dirty = true)")
    ...
    if args.fetch_missing_hours_for_dirty_addresses:
        ...
        print(f"Processing missing hourly weather data for dirty addresses...")
        process_dirty_locations(api_key, dry_run=args.dry_run, 
    ...
    if args.fetch_missing_hours_for_address:
        ...
        try:
            ...
            # Use the shared function to fetch missing hours for the address
            success = fetch_missing_hours_for_single_address(
                db_conn, api_key, target_address, dry_run=args.dry_run, 
                max_days_per_request=args.max_days_per_request, 
                include_inactive=args.include_inactive)
...
#------- END NEW --------
    ...
    if args.run_for_all_locations:
        ...
        if args.address or args.start_date or args.end_date or args.tzoffset or args.get_missing_hours or args.get_missing_tz or args.update_for_display_group_id or args.list_display_group_ids_missing_hours or args.report_missing_hours_by_address or args.output_json or args.run_for_active_locations or args.fetch_missing_hours or args.fetch_missing_hours_for_address or args.fetch_from_report or args.fetch_missing_hours_for_dirty_addresses:
            print("Error: --run-for-all-locations cannot be used with --address, --start-date, --end-date, --tzoffset, --get-missing-hours, --get-missing-tz, --update-for-display-group-id, --list-display-group-ids-missing-hours, --report-missing-hours-by-address, --output-json options, --run-for-active-locations, --fetch-missing-hours-for-dirty-addresses, or the new fetch options.")
            return
    ...
    if args.update_for_display_group_id:
        if args.address or args.start_date or args.end_date or args.tzoffset or args.get_missing_hours or args.get_missing_tz or args.list_display_group_ids_missing_hours or args.report_missing_hours_by_address or args.output_json or args.run_for_all_locations or args.run_for_active_locations or args.fetch_missing_hours or args.fetch_missing_hours_for_address or args.fetch_from_report or args.fetch_missing_hours_for_dirty_addresses:
            print("Error: --update-for-display-group-id cannot be used with --address, --start-date, --end-date, --tzoffset, --get-missing-hours, --get-missing-tz, --list-display-group-ids-missing-hours, --report-missing-hours-by-address, --output-json options, --run-for-all-locations, --run-for-active-locations, --fetch-missing-hours-for-dirty-addresses, or the new fetch options.")
            return
