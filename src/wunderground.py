...
def get_temperature(location, date, debug=False):
    url = f"http://api.weather.com/v3/wx/conditions/historical/{date}/q/{location}.json?apiKey={API_KEY}"
    ...
    avg_temp = data['observations'][0]['metric']['temp']
    max_temp = data['observations'][0]['metric']['maxTemp']
    min_temp = data['observations'][0]['metric']['minTemp']
